const playList = [
    {
      title: 'La Ocasion',
      artist: 'De La Ghetto',
      img: 'assets/foto-resena-min.jpg',
      song: 'music/La Ocasión - De La Ghetto, Arcangel, Ozuna, Anuel Aa, Dj Luian, Mambo Kingz [Video Oficial] - Hear This Music (youtube).mp3'
    },
    {
      title: 'Mi fanatica',
      artist: 'De La Ghetto',
      img: 'assets/channels4_profile.jpg',
      song: 'music/Mi Fanatica - De La Ghetto (youtube).mp3'
    },
    {
      title: 'Dices',
      artist: 'De La Ghetto',
      img: 'assets/de la .jpg',
      song: 'music/De la Ghetto - Dices [Official Video] - De La Ghetto (youtube).mp3'
    },
    {
      title: 'Sensacion Del Bloque',
      artist: 'De La Ghetto',
      img: 'assets/delaghettoent2.jpg',
      song: 'music/Sensacion Del Bloque - De La Ghetto (youtube).mp3'
    },
    {
      title: 'La Groupie',
      artist: 'De La Ghetto',
      img: 'assets/de-la-ghetto-entrevista-espana-2023-5.jpg',
      song: 'music/De La Ghetto - La Groupie ft. Ñejo, Luigi 21+, Nicky Jam & Ñengo Flow [Official Audio] - De La Ghetto (youtube).mp3'
    },
];